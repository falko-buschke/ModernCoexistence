...
resource.level <- seq(10,120,by=10)
...
write.table(rich.mat,file= "Processed_data/Laikipia_SpRichIter100.txt",quote=T,sep="\t",row.names=F,col.names=T)

colnames(sp.prob) <- sp.name
write.table(sp.prob,file= "Processed_data/Laikipia_SpProbIter100.txt",quote=T,sep="\t",row.names=F,col.names=T)

library(vioplot)
library(RColorBrewer)

# Set up the plot and dimensions
#png(filename="Laikipia_cascade.png",width=28,height=12,units="cm",res=300)
...
X <- vioplot(rich.mat, ylim=c(0,14),las=1, main="", ylab="Maximum species richness", col=rgb(0.8,0.8,0.8,1),
	xlab="Number of resources",cex.axis=1, cex.lab= 1,border=NA, xaxt="n",wex=1.2, rectCol="black",
	lineCol="black", colMed="red", cexMed=1.5, xlim=c(-.15,13))
axis(1, at =seq(0,12,by=2), labels=c("0","20","40","60","80","100","120"))
points(c(0,seq(1,12,by=1),12.1),c(0,X$median,6),pch=16,col=rgb(0.7,0,0,1), cex=1.3)
mtext("(a)",cex=1.3, side = 3, adj = 0.03, line = -1.25,font=1)

IV <- c(0,seq(1,12,by=1),12.6)
DV <- c(0,X$median,8)
...
#display.brewer.all()
pres <- c(0,1,1,0,1,0,1,0,0,1,1,0)
...
png(filename="Figures/Supp/Figure S16.png",width=28,height=20,units="cm",res=300)
...
png(filename="Figures/Supp/Figure S13.png",width=28,height=20,units="cm",res=300)
par(mai=c(.7,.7,0.25,0.1))
...
png(filename="Figures/Supp/Figure S10.png",width=28,height=14,units="cm",res=300)
par(mfrow=c(1,2))
par(mai=c(.8,.8,0.1,0.1))
