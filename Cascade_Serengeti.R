...
resource.level <- seq(10,90,by=10)
...
write.table(rich.mat,file= "Processed_data/Serengeti_SpRichIter100.txt",quote=T,sep="\t",row.names=F,col.names=T)

colnames(sp.prob) <- sp.name
write.table(sp.prob,file= "Processed_data/Serengeti_SpProbIter100.txt",quote=T,sep="\t",row.names=F,col.names=T)
...
#png(filename="Serengeti_cascade.png",width=28,height=12,units="cm",res=300)
...
X <- vioplot(rich.mat, ylim=c(0,8),las=1, main="", ylab="Maximum species richness", col=rgb(0.8,0.8,0.8,1),
	xlab="Number of resources",cex.axis=1, cex.lab= 1,border=NA, xaxt="n",wex=1.2, rectCol="black",
	lineCol="black", colMed="red", cexMed=1.5, xlim=c(-.15,10))
axis(1, at =seq(0,10,by=2), labels=c("0","20","40","60","80","100"))
points(c(0,seq(1,9,by=1),9.1),c(0,X$median,4),pch=16,col=rgb(0.7,0,0,1), cex=1.3)
mtext("(a)",cex=1.3, side = 3, adj = 0.03, line = -1.25,font=1)

IV <- c(0,seq(1,9,by=1),9.1)
DV <- c(0,X$median,4)
...
#display.brewer.all()
pres <- c(1,0,1,1,0,0,1,0) # Correct for Seringeti
cols2 <- brewer.pal(n = 8, name = "Paired")
for (nsp in 1:8){
...
legend("topright",cex=0.7,ncol=3,lwd=2,legend=sp.name,col=cols2,lty=1, bty="n")
...
png(filename="Figures/Supp/Figure S15.png",width=20,height=20,units="cm",res=300)
...
for (j in 1:9){
  ...
  #     xlab="Mean Bluthgen d' (Plants)", ylim=c(0,10), xlim=c(min(D.mat.p[,j]), max(D.mat.p[,j])), main=paste("Number of plant resources =",mean(res.mat[,j])))
  plot(D.mat.p[,j],rich.mat[,j],  pch=21, bg=cols2[res.mat[,j]/10], cex=1.5,las=1, ylab="Herbivore richness", 
...
png(filename="Figures/Supp/Figure S12.png",width=20,height=20,units="cm",res=300)
par(mai=c(.7,.7,0.25,0.1))
...
for (j in 1:9){
 ...
 # plot(D.mat.m[,j],rich.mat[,j],  pch=21, bg=cols2[res.mat[,j]/10], cex=1.5,las=1, ylab="Herbivore richness", 
  #     xlab="Mean Bluthgen d'(Herbivores)", ylim=c(0,10), xlim=c(min(D.mat.m[,j]), max(D.mat.m[,j])), main=paste("Number of plant resources =",mean(res.mat[,j])))
...
png(filename="Figures/Supp/Figure S9.png",width=28,height=14,units="cm",res=300)
par(mfrow=c(1,2))
par(mai=c(.8,.8,0.1,0.1))
